@php
$hour = date('H');
$theme = ($hour >= 18 || $hour < 6) ? 'dark' : 'light' ; $headers=['Id', 'Nombre' , 'Apellido' , 'Telefono' , 'Genero'
    , 'Rol' , 'Email' , 'Acciones' ]; $rows=$users->map(function($user) {
    return [
    'id' => $user->id,
    'nombre' => $user->name,
    ...
    <x-app-layout>
        ...
        <div class="py-12">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <x-table :headers="$headers" :rows="$rows" :theme="$theme" resource="usuario" />
                </div>
            </div>
        </div>

        <!-- Modal de edición -->
        <x-modal id="editModal" title="Editar Usuario" :theme="$theme">
            <form id="edit-form" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="mb-4">
                        <label for="edit-apellido"
                            class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Apellido</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="mb-4">
                        <label for="edit-genero"
                            class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Genero</label>
                ...
                <div class="mb-4">
                    <label for="edit-password"
                        class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Password</label>
                ...
                <div class="mb-4">
                    <label for="edit-rol"
                        class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Rol</label>
                    <select id="edit-rol" name="rol_id"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 {{ $theme === 'dark' ? 'dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white' : '' }}"
                        required>
        ...
        <!-- Modal de creación -->
        <x-modal id="createModal" title="Crear Usuario" :theme="$theme">
            <form id="create-form" method="POST" action="{{ route('users.store') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="mb-4">
                        <label for="create-apellido"
                            class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Apellido</label>
                        <input type="text" id="create-apellido" name="apellido"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 {{ $theme === 'dark' ? 'dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white' : '' }}"
                            required>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    <label for="create-rol"
                        class="block mb-2 text-sm font-medium text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">Rol</label>
                    <select id="create-rol" name="rol_id"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 {{ $theme === 'dark' ? 'dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white' : '' }}"
                        required>
                        @foreach($roles as $role)
                        <option value="{{ $role->id }}">{{ $role->nombre }}</option>
        ...
        <x-confirm-modal id="deleteModal" title="¿Estás seguro de que quieres eliminar este usuario?"
            :theme="$theme">
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', (event) => {
                        ...
                        document.getElementById('edit-rol').value = roles.find(role => role.nombre === data.rol).id; 
                        editForm.setAttribute('action', `/users/${data.id}`);
                    });
                });
