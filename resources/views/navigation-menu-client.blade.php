    ...
    <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl p-4">
        ...
        <div class="flex items-center md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
            ...
            <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow {{ $theme === 'dark' ? 'dark:bg-gray-700 dark:divide-gray-600' : '' }}" id="user-dropdown">
                <div class="px-4 py-3">
                    <span class="block text-sm text-gray-900 {{ $theme === 'dark' ? 'dark:text-white' : '' }}">{{ Auth::user()->name }}</span>
                    <span class="block text-sm text-gray-500 truncate {{ $theme === 'dark' ? 'dark:text-gray-400' : '' }}">{{ Auth::user()->email }}</span>
                </div>
                <ul class="py-2" aria-labelledby="user-menu-button">
                    <li>
                        <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 {{ $theme === 'dark' ? 'dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white' : '' }}">Home</a>
                    </li>
                    <li>
        ...
        <div id="mega-menu-full" class="items-center justify-between font-medium hidden w-full md:flex md:w-auto md:order-1">
            <ul class="flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white {{ $theme === 'dark' ? 'dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700' : '' }}">
                <li>
                    <a href="{{ route('dashboard') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 {{ $theme === 'dark' ? 'dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-blue-500 md:dark:hover:bg-transparent dark:border-gray-700' : '' }}" aria-current="page">Home</a>
                </li>
                <li>
                    <a href="{{ route('booking-client') }}" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0 {{ $theme === 'dark' ? 'dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-blue-500 md:dark:hover:bg-transparent dark:border-gray-700' : '' }}" aria-current="page">Reservas</a>
                </li>
            </ul>
        </div>
